...
var Expressions = {     
	...
	{                        
		...
		//could be func
		return { name: ast[1][1] , reference: v, params: ast[2] };
	...
	{                   
		var vars = ast[1],
     	...
     	vars.forEach(function(vr, i)
     	{
			...
			walker.symbolTable().set(val.name, val.value, false);
		});          
	...
	{                       
		var property = ast[1].walker._prepare(),  
		chain = [],
		ref; 
	...
	{
		return { name: ast[2], value: ast[3], reference: walker.symbolTable().get(ast[2].walker._prepare().name, walker) };	
	},
...
var _init = function(ast, parent, symbolTable)
{
...
{   
	...
	var walker = ast.walker = {  
		...
		_prepare: function()
		{         
			if(__prepared) return walker;
			__prepared= true;

			if(walker.expr) copyTo(walker.expr(ast, walker), walker);
		...
		{               
			for(var i = 0, n = ast.length; i < n; i++)
			{
				...
				if(handlers[v] && v != '__proto__' && v != '__defineGetter__' && v  != '__defineGetter__')
				{
...
{   
	...
	try
	{
		return aster(ast, symbolTable)._prepare();
	}
	catch(e)
	{
		console.log(e.stack);
